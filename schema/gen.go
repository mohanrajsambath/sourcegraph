...
//go:generate $PWD/.bin/go-jsonschema-compiler -o schema.go -pkg schema awsCodeCommit.schema.json bitbucketServer.schema.json critical.schema.json site.schema.json settings.schema.json github.schema.json gitlab.schema.json gitolite.schema.json otherExternalService.schema.json phabricator.schema.json
...
//go:generate env GO111MODULE=on go run stringdata.go -i bitbucketServer.schema.json -name BitbucketServerSchemaJSON -pkg schema -o bitbucket_server_stringdata.go
